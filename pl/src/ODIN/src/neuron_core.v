...
module neuron_core #(
	...
	input  wire                 REGMEM_PROPAGATE_UNMAPPED_SYN,
    ...
    // input  wire [      2*M-1:0] CTRL_REGMEM_ADDR,
    ...
    output reg  [   31:0] BRAM_MEM_NEURAM_DOUT,
    output reg            BRAM_MEM_NEURAM_RD
);
    ...
    assign syn_weight      = |CTRL_NEUR_VIRTS ? CTRL_NEUR_VIRTS[4:2] : (syn_weight_int[2:0] & {3{syn_weight_int[3] | REGMEM_PROPAGATE_UNMAPPED_SYN}});
    ...
    //         assign neuron_data[M*i+M-1:M*i] = REGMEM_GATE_ACTIVITY_sync
    //                                         ? ((i == CTRL_REGMEM_ADDR[2*M-1:M])
    ...
    wire en = BRAM_MEM_EN && (BRAM_MEM_ADDR[16:15] == 2'b01) && (BRAM_MEM_ADDR[1:0] == 2'b00);
    wire [31:0] we;
    generate
    ...
    always @(posedge CLK, negedge RSTN_syncn) 
        if (!RSTN_syncn) BRAM_MEM_NEURAM_RD <= 0;
        else             BRAM_MEM_NEURAM_RD <= en && ~(|BRAM_MEM_WE);
    
    generate
        for (i=0; i<4; i=i+1) begin
            assign neuron_data[32*i+32-1:32*i] = REGMEM_GATE_ACTIVITY_sync
                                            ? ((i == addr[M+1:M])
                                                     ? (neuron_data_int[32*i+32-1:32*i] & ~we) | din
                                                     : NEUR_STATE[32*i+32-1:32*i])
                                            : neuron_data_int[32*i+32-1:32*i];
        end
    endgenerate

    always @(*) begin
        BRAM_MEM_NEURAM_DOUT = 32'b0;
        for (j = 0; j < 4; j = j + 1) begin
            if (j[1:0] == addr[M+1:M])
                BRAM_MEM_NEURAM_DOUT = NEUR_STATE[32*j +: 32];
        end
    end

    NEURAM neuram_0 (
        .clka(CLK),    // input wire clka
        .ena(REGMEM_GATE_ACTIVITY_sync ? en : CTRL_NEURMEM_CS),      // input wire ena
        .wea(REGMEM_GATE_ACTIVITY_sync ? |we : CTRL_NEURMEM_WE),      // input wire [0 : 0] wea
        .addra(REGMEM_GATE_ACTIVITY_sync ? addr : CTRL_NEURMEM_ADDR),  // input wire [7 : 0] addra
        .dina(neuron_data),    // input wire [127 : 0] dina
        .douta(NEUR_STATE)  // output wire [127 : 0] douta
    );
