...
module aer_out #(
    ...
    input  wire           REGMEM_OUT_AER_MONITOR_EN,
    input  wire [  M-1:0] REGMEM_MONITOR_NEUR_ADDR,
    input  wire [  M-1:0] REGMEM_MONITOR_SYN_ADDR,
    input  wire           REGMEM_AER_SRC_CTRL_nNEUR,
   ...
   assign rst_activity = RST || REGMEM_GATE_ACTIVITY_sync;
   
   assign monitored_neuron_popped  = CTRL_AEROUT_POP_NEUR && (SCHED_DATA_OUT[M-1:0] == REGMEM_MONITOR_NEUR_ADDR);
   
   assign neuron_state_event       = REGMEM_OUT_AER_MONITOR_EN && ((CTRL_NEURMEM_CS  && CTRL_NEURMEM_WE  && (CTRL_NEURMEM_ADDR  == REGMEM_MONITOR_NEUR_ADDR)) || (monitored_neuron_popped && REGMEM_AER_SRC_CTRL_nNEUR));
   assign synapse_state_event_cond = REGMEM_OUT_AER_MONITOR_EN &&   CTRL_SYNARRAY_CS && CTRL_SYNARRAY_WE && (CTRL_SYNARRAY_ADDR == {REGMEM_MONITOR_SYN_ADDR, REGMEM_MONITOR_NEUR_ADDR[7:3]});
   ...
   always @(posedge CLK, posedge rst_activity) begin
    ...
    assign synapse_state_int = SYNARRAY_WDATA >> ({2'b0,REGMEM_MONITOR_NEUR_ADDR[2:0]} << 2);
    ...
    always @(posedge CLK, posedge rst_activity) begin
		if (rst_activity) begin
		...
		end else if (~REGMEM_OUT_AER_MONITOR_EN) begin
            do_neuron0_transfer     <= 1'b0;
            do_neuron1_transfer     <= 1'b0;
            do_synapse_transfer     <= 1'b0;
            synapse_state_event_del <= 1'b0;
            if ((REGMEM_AER_SRC_CTRL_nNEUR ? CTRL_AEROUT_POP_NEUR : NEUR_EVENT_OUT[6]) && ~AEROUT_ACK_sync) begin
                AEROUT_ADDR      <= REGMEM_AER_SRC_CTRL_nNEUR ? SCHED_DATA_OUT[M-1:0] : CTRL_NEURMEM_ADDR;
                AEROUT_REQ       <= 1'b1;
            ...
            if (AEROUT_ACK_sync_negedge) begin
                ...
                AEROUT_ADDR             <= synapse_state_event ? {4'b1111,synapse_state_int[3:0]}
                                                               : {(REGMEM_AER_SRC_CTRL_nNEUR ? monitored_neuron_popped : NEUR_EVENT_OUT[6]),NEUR_STATE_MONITOR[14:8]};
                AEROUT_REQ              <= 1'b1;
                AEROUT_CTRL_BUSY        <= 1'b1;
