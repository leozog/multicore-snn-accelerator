...
module synaptic_core #(
    ...
    input  wire [  N-1:0] REGMEM_SYN_SIGN, 
    input  wire           REGMEM_UPDATE_UNMAPPED_SYN,
    ...
    output wire [   31:0] BRAM_MEM_SYNRAM_DOUT,
    output reg            BRAM_MEM_SYNRAM_RD
);
    ...
    generate
        for (i=0; i<8; i=i+1) begin
            ...
            sdsp_update #(
                ...
                // Inputs
                    // General
                .SYN_PRE(CTRL_PRE_EN[i] & (REGMEM_UPDATE_UNMAPPED_SYN | SYNARRAY_RDATA[(i<<2)+3])),
    ...
    //         // assign SYNARRAY_WDATA[(i<<3)+7:(i<<3)] = REGMEM_GATE_ACTIVITY_sync
    //         //                                        ?
    //         //                                            ((i == CTRL_REGMEM_ADDR[14:13])
    ...
    wire en = BRAM_MEM_EN && (BRAM_MEM_ADDR[16:15] == 2'b10) && (BRAM_MEM_ADDR[1:0] == 2'b00);
    wire [31:0] we;
    generate
        for (i=0; i<4; i=i+1) begin
    ...
    always @(posedge CLK, negedge RSTN_syncn) 
        if (!RSTN_syncn) BRAM_MEM_SYNRAM_RD <= 0;
        else             BRAM_MEM_SYNRAM_RD <= en && ~(|BRAM_MEM_WE);

    assign BRAM_MEM_SYNRAM_DOUT = SYNARRAY_RDATA;

    assign SYNARRAY_WDATA = REGMEM_GATE_ACTIVITY_sync
                            ? (SYNARRAY_WDATA_int & ~we) | din
                            : SYNARRAY_WDATA_int;
    ...
    SYNRAM synram_0 (
        ...
        .ena(REGMEM_GATE_ACTIVITY_sync ? en : CTRL_SYNARRAY_CS),        // input wire ena
        .wea(REGMEM_GATE_ACTIVITY_sync ? |we : CTRL_SYNARRAY_WE),       // input wire [0 : 0] wea
        .addra(REGMEM_GATE_ACTIVITY_sync ? addr : CTRL_SYNARRAY_ADDR),  // input wire [12 : 0] addra
        .dina(SYNARRAY_WDATA),                                          // input wire [31 : 0] dina
        .douta(SYNARRAY_RDATA)                                          // output wire [31 : 0] douta
    );
    
    assign {syn_sign_dummy,SYN_SIGN} = REGMEM_SYN_SIGN >> CTRL_SYNARRAY_ADDR[12:5];
