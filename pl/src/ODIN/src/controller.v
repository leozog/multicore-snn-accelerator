...
module controller #(
    ...
    input  wire           REGMEM_GATE_ACTIVITY, 
    output reg            REGMEM_GATE_ACTIVITY_sync,
    input  wire [  M-1:0] REGMEM_MONITOR_NEUR_ADDR,
	input  wire           REGMEM_SDSP_ON_SYN_STIM,
...
// 	end
   ...
   always @(posedge CLK, posedge RST) begin
		if(RST) begin
 			AERIN_REQ_sync_int              <= 1'b0;
 			AERIN_REQ_sync	                <= 1'b0;
            REGMEM_GATE_ACTIVITY_sync       <= 1'b0;
		end
		else begin
 			AERIN_REQ_sync_int              <= AERIN_REQ;
 			AERIN_REQ_sync	                <= AERIN_REQ_sync_int;
            REGMEM_GATE_ACTIVITY_sync       <= REGMEM_GATE_ACTIVITY & ((nextstate == WAIT) | REGMEM_GATE_ACTIVITY_sync);
		end
	end
	...
	always @(*)
		case(state)
			WAIT 		:	if      (AEROUT_CTRL_BUSY)                                                          nextstate = WAIT;
                            ...
                            //     else                                                                            nextstate = WAIT;
                            else if (REGMEM_GATE_ACTIVITY_sync)                                                 nextstate = WAIT_REGDN;
                            else
                                if (SCHED_FULL)
			...
			WAIT_REGDN  :   if      (~REGMEM_GATE_ACTIVITY_sync)                                                   nextstate = WAIT;
							else					                                                            nextstate = WAIT_REGDN;
    ...
    assign CTRL_AEROUT_POP_NEUR = (state == POP_NEUR) && (neur_cnt == REGMEM_MONITOR_NEUR_ADDR) && ctrl_cnt[0];
 
    assign ODIN_BUSY = (state != WAIT || nextstate != WAIT || AEROUT_CTRL_BUSY || !SCHED_EMPTY);
    ...
    always @(*) begin
        ...
        if (state == TREF) begin
            ...
            CTRL_PRE_EN         = {7'b0, REGMEM_SDSP_ON_SYN_STIM} << AERIN_ADDR[2:0];
            CTRL_NEUR_EVENT     = 1'b1;
            CTRL_SYNARRAY_CS    = 1'b1;
            CTRL_NEURMEM_CS     = 1'b1;
            if (ctrl_cnt == 32'd0) begin
