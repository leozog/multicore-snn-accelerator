...
module scheduler #(
    ...
    input  wire                 REGMEM_OPEN_LOOP,
    input  wire [         19:0] REGMEM_BURST_TIMEREF,
    ...
    reg                    REGMEM_OPEN_LOOP_sync_int, REGMEM_OPEN_LOOP_sync;
    ...
    // Sync barrier from REG

    always @(posedge CLK, negedge RSTN) begin
        if(~RSTN) begin
            REGMEM_OPEN_LOOP_sync_int  <= 1'b0;
            REGMEM_OPEN_LOOP_sync	    <= 1'b0;
        end
        else begin
            REGMEM_OPEN_LOOP_sync_int  <= REGMEM_OPEN_LOOP;
            REGMEM_OPEN_LOOP_sync	    <= REGMEM_OPEN_LOOP_sync_int;
        end
    end

    // Splitting event_out into FIFO push commands

    assign spike_in  = (~REGMEM_OPEN_LOOP_sync & NEUR_EVENT_OUT[6]) | CTRL_SCHED_EVENT_IN[6];
    ...
    assign timestamp_next = (priority_cnt == REGMEM_BURST_TIMEREF);
    ...
    assign rst_priority = ~RSTN || REGMEM_OPEN_LOOP_sync || (~|REGMEM_BURST_TIMEREF);
